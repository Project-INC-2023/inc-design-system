...
const alertVariants = cva(
  "p-3 rounded-lg w-full flex items-center px-2 justify-between",
  {
    variants: {
      variant: {
...
const Alert = React.forwardRef<HTMLDivElement, AlertProps>(
  (
    ...
    if (variant === "info") {
      ...
      icon = (
        <CircleAlert size={24} className="fill-warning stroke-warning-accent" />
      );
    } else if (variant === "success") {
      icon = (
        <CircleCheck size={24} className="fill-success stroke-success-accent" />
      );
    }
